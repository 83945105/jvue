<template>
  <el-select ref="select"
    ...
    <template v-for="(group, $index) in data__">
      <slot>
        <el-option-group :key="group.label + '_' + $index"
                         :label="group.label"
                         :disabled="group.disabled"
        >
          <template v-for="(option, $idx) in (group.data || [])">
            <el-option :key="option.value + '_' + $index + '_' + $idx"
                       :label="option.label"
                       :value="option.value">
            </el-option>
          </template>
...
<script>
  ...
  export default {
    ...
    computed: {
      dataProp__() {
        return {
          groupLabel: this.dataProp.groupLabel || 'label',
          groupDisabled: this.dataProp.groupDisabled || 'disabled',
          groupData: this.dataProp.groupData || 'data',
      ...
      data__() {
        return (this.data || []).map(group => {
          return {
            label: group[this.dataProp__.groupLabel],
            disabled: !!group[this.dataProp__.groupDisabled],
            data: (group[this.dataProp__.groupData] || []).map(option => {
              return {
